...
// =============================================================================
// SIZE-PROPORTIONAL SHAPES  (area ~ sqrt(size) so perception is linear)
// r = clamp(sqrt(size) * 1.6, 3, 11)
// =============================================================================
function scaleR(size, minR=3, maxR=11) {
  if (!size || size <= 0) return minR;
  return Math.max(minR, Math.min(maxR, Math.sqrt(size) * 1.6));
}
function scaleTri(size, minH=5, maxH=13) {
  if (!size || size <= 0) return minH;
  return Math.max(minH, Math.min(maxH, Math.sqrt(size) * 1.4));
}

function CircleDot({ cx, cy, fill, payload }) {
  if (cx == null || cy == null) return null;
  const r = scaleR(payload && payload.size);
  return (
    <g>
      <circle cx={cx} cy={cy} r={r}       fill={fill} fillOpacity={0.85} stroke="#0f172a" strokeWidth={1} />
      <circle cx={cx} cy={cy} r={r + 2.5} fill="none" stroke={fill} strokeWidth={0.8} strokeOpacity={0.4} />
    </g>
  );
}
function BuyOrderDot({ cx, cy, payload }) {
  if (cx == null || cy == null) return null;
  const h = scaleTri(payload && payload.size);
  const w = h * 0.75;
  return <polygon points={cx+","+(cy+h)+" "+(cx-w)+","+(cy-h*0.4)+" "+(cx+w)+","+(cy-h*0.4)} fill="#22c55e" stroke="#0f172a" strokeWidth={0.5} />;
}
function SellOrderDot({ cx, cy, payload }) {
  if (cx == null || cy == null) return null;
  const h = scaleTri(payload && payload.size);
  const w = h * 0.75;
  return <polygon points={cx+","+(cy-h)+" "+(cx-w)+","+(cy+h*0.4)+" "+(cx+w)+","+(cy+h*0.4)} fill="#f43f5e" stroke="#0f172a" strokeWidth={0.5} />;
}
...
// =============================================================================
const PriceChart = memo(function PriceChart({ sym, parsedData, hidden, refArea, showPosSeries }) {
  const touch      = (parsedData.touch[sym]      || []);
  const placedBids = (parsedData.placedBids[sym] || []);
  const placedAsks = (parsedData.placedAsks[sym] || []);
  const buyFills   = (parsedData.fills[sym]      || []).filter(f => f.side === "buy");
  const sellFills  = (parsedData.fills[sym]      || []).filter(f => f.side === "sell");

  // all symbols position data for secondary axis (shown on global chart)
  const allSymbols = parsedData.symbols;

  const priceVals = useMemo(function() {
    const v = [];
    if (!hidden.mid)  touch.forEach(d => d.mid  != null && v.push(d.mid));
    if (!hidden.bid1) touch.forEach(d => d.bid1 != null && v.push(d.bid1));
    if (!hidden.bid2) touch.forEach(d => d.bid2 != null && v.push(d.bid2));
    if (!hidden.bid3) touch.forEach(d => d.bid3 != null && v.push(d.bid3));
    if (!hidden.ask1) touch.forEach(d => d.ask1 != null && v.push(d.ask1));
    if (!hidden.ask2) touch.forEach(d => d.ask2 != null && v.push(d.ask2));
    if (!hidden.ask3) touch.forEach(d => d.ask3 != null && v.push(d.ask3));
    if (!hidden.pnl)  touch.forEach(d => d.pnl  != null && v.push(d.pnl));
    if (!hidden.placedBid) placedBids.forEach(d => v.push(d.price));
    if (!hidden.placedAsk) placedAsks.forEach(d => v.push(d.price));
    if (!hidden.buyFill)  buyFills.forEach(d => v.push(d.price));
    if (!hidden.sellFill) sellFills.forEach(d => v.push(d.price));
    return v;
  }, [touch, placedBids, placedAsks, buyFills, sellFills, hidden]);

  const priceDom = useMemo(function() {
    if (!priceVals.length) return ["auto", "auto"];
    const lo = Math.min.apply(null, priceVals), hi = Math.max.apply(null, priceVals);
  ...
  const posDom = useMemo(function() {
    ...
    allSymbols.forEach(function(s) {
      if (!hidden["pos_"+s]) (parsedData.position[s] || []).forEach(d => v.push(d.pos));
    });
  ...
  }, [parsedData, allSymbols, hidden, showPosSeries]);

  const xDom = useMemo(function() {
    if (!touch.length) return ["auto","auto"];
    return [touch[0].ts, touch[touch.length-1].ts];
  }, [touch]);

  const ttStyle = { background:"#0f172a", border:"1px solid #334155", borderRadius:4, fontSize:11 };

  return (
    <ResponsiveContainer width="100%" height="100%">
      <ComposedChart margin={{ top:4, right: showPosSeries ? 52 : 12, bottom:4, left:4 }}>
        <CartesianGrid strokeDasharray="2 4" stroke="#1e293b" />
        <XAxis dataKey="ts" type="number" domain={xDom} tick={{ fill:"#475569", fontSize:10 }} tickLine={false} allowDuplicatedCategory={false} />
        <YAxis yAxisId="price" domain={priceDom} tick={{ fill:"#475569", fontSize:10 }} tickLine={false} width={56} tickFormatter={v => v.toFixed(1)} />
        {showPosSeries && (
          <YAxis yAxisId="pos" orientation="right" domain={posDom} tick={{ fill:"#64748b", fontSize:10 }} tickLine={false} width={44}
            label={{ value:"pos", angle:90, position:"insideRight", fill:"#475569", fontSize:9, offset:8 }} />
        )}
        <Tooltip contentStyle={ttStyle} labelStyle={{ color:"#94a3b8" }} itemStyle={{ color:"#cbd5e1" }}
          labelFormatter={v => "ts: "+v} isAnimationActive={false} />

        {/* price lines */}
        {!hidden.mid    && <Line yAxisId="price" data={touch} dataKey="mid"    stroke="#60a5fa" dot={false} strokeWidth={1.5} isAnimationActive={false} legendType="none" />}
        {!hidden.movAvg && <Line yAxisId="price" data={touch} dataKey="movAvg" stroke="#f97316" dot={false} strokeWidth={1}   isAnimationActive={false} strokeDasharray="5 3" legendType="none" />}
        {!hidden.bid1   && <Line yAxisId="price" data={touch} dataKey="bid1"   stroke="#34d399" dot={false} strokeWidth={1}   isAnimationActive={false} strokeDasharray="4 2" legendType="none" />}
        {!hidden.bid2   && <Line yAxisId="price" data={touch} dataKey="bid2"   stroke="#34d39966" dot={false} strokeWidth={0.8} isAnimationActive={false} strokeDasharray="3 3" legendType="none" />}
        {!hidden.bid3   && <Line yAxisId="price" data={touch} dataKey="bid3"   stroke="#34d39933" dot={false} strokeWidth={0.8} isAnimationActive={false} strokeDasharray="2 4" legendType="none" />}
        {!hidden.ask1   && <Line yAxisId="price" data={touch} dataKey="ask1"   stroke="#fb7185" dot={false} strokeWidth={1}   isAnimationActive={false} strokeDasharray="4 2" legendType="none" />}
        {!hidden.ask2   && <Line yAxisId="price" data={touch} dataKey="ask2"   stroke="#fb718566" dot={false} strokeWidth={0.8} isAnimationActive={false} strokeDasharray="3 3" legendType="none" />}
        {!hidden.ask3   && <Line yAxisId="price" data={touch} dataKey="ask3"   stroke="#fb718533" dot={false} strokeWidth={0.8} isAnimationActive={false} strokeDasharray="2 4" legendType="none" />}
        {!hidden.pnl    && <Line yAxisId="price" data={touch} dataKey="pnl"    stroke="#e2e240" dot={false} strokeWidth={1.2} isAnimationActive={false} legendType="none" />}

        {/* placed orders: triangles */}
        {!hidden.placedBid && <Scatter yAxisId="price" data={placedBids} dataKey="price" name="placed bid" fill="#22c55e" shape={<BuyOrderDot />} isAnimationActive={false} legendType="none" />}
        {!hidden.placedAsk && <Scatter yAxisId="price" data={placedAsks} dataKey="price" name="placed ask" fill="#f43f5e" shape={<SellOrderDot />} isAnimationActive={false} legendType="none" />}

        {/* fills: circles with ring */}
        {!hidden.buyFill  && <Scatter yAxisId="price" data={buyFills}  dataKey="price" fill="#a3e635" shape={<CircleDot fill="#a3e635" />} isAnimationActive={false} legendType="none" />}
        {!hidden.sellFill && <Scatter yAxisId="price" data={sellFills} dataKey="price" fill="#fb923c" shape={<CircleDot fill="#fb923c" />} isAnimationActive={false} legendType="none" />}

        {/* position lines -- all symbols on right axis */}
        {showPosSeries && allSymbols.map(function(s, i) {
          const posData = parsedData.position[s] || [];
          if (isHidden(hidden, "pos_"+s) || !posData.length) return null;
          return <Line key={"pos_"+s} yAxisId="pos" data={posData} dataKey="pos" stroke={posColor(i)}
            dot={false} strokeWidth={1.5} type="stepAfter" isAnimationActive={false} legendType="none" />;
        })}

        {/* per-symbol pnl lines -- price axis, hidden by default */}
        {allSymbols.map(function(s, i) {
          if (isHidden(hidden, "pnl_"+s, true)) return null;
          const pnlData = parsedData.touch[s] || [];
          if (!pnlData.length) return null;
          return <Line key={"pnl_"+s} yAxisId="price" data={pnlData} dataKey="pnl" stroke={pnlColor(i)}
            dot={false} strokeWidth={1.2} isAnimationActive={false} legendType="none" />;
        })}
...
function DataTable({ sym, parsedData, tsRange }) {
  ...
  const rows = useMemo(function() {
    ...
    return touch.map(function(d) {
      return {
        ts: d.ts, mid: d.mid, bid1: d.bid1, ask1: d.ask1, movAvg: d.movAvg,
        pos:  posMap[d.ts] != null ? posMap[d.ts] : "",
        pBid: (pbMap[d.ts] || []).join(" "),
        pAsk: (paMap[d.ts] || []).join(" "),
        fills: (fillsMap[d.ts] || []).map(f => (f.side==="buy"?"B":"S")+f.price+"x"+f.size).join(" "),
      };
    });
  ...
  const cols = [
    ...
    { h:"p.bid", get: r => r.pBid, color:"#22c55e" },
    { h:"p.ask", get: r => r.pAsk, color:"#f43f5e" },
    { h:"fills", get: r => r.fills,color:"#e2e8f0" },
  ];
...
export default function App() {
  ...
  return (
    <div style={{ background:"#020817", color:"#e2e8f0", height:"100vh", display:"flex", flexDirection:"column", fontFamily:"'JetBrains Mono','Fira Code',monospace", overflow:"hidden" }}>
      ...
      {/* top bar */}
      <div style={{ padding:"8px 16px", borderBottom:"1px solid #0f172a", display:"flex", alignItems:"center", gap:8, flexShrink:0, flexWrap:"wrap" }}>
        ...
        <button onClick={() => fileRef.current && fileRef.current.click()} style={{ ...btnBase, background:"#0ea5e9", color:"#020817" }}>
          Load File
        </button>
        ...
        <input
          ...
          style={{ background:"#0f172a", color:"#94a3b8", border:"1px solid #1e293b", borderRadius:3, padding:"4px 8px", fontSize:10, fontFamily:"inherit", outline:"none", width:220 }}
        />
        ...
        <span style={{ fontSize:9, color:"#334155", maxWidth:180, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{fileName}</span>
        ...
        {/* reset */}
        <button onClick={reset} style={{ ...btnBase, background:"#be123c", color:"#fff" }} title="Clear all data and reset view">
          Reset
        </button>
      </div>
        ...
        <div ref={horiz.ref} style={{ flex:1, minHeight:0, display:"flex", flexDirection:"row", overflow:"hidden" }}>
          ...
          <div style={{ width:horiz.pct+"%", minWidth:0, display:"flex", flexDirection:"column", padding:"8px 0 8px 16px" }}>
            ...
            <div style={{ flex:1, minHeight:0 }}>
              {sym && <PriceChart sym={sym} parsedData={localParsed} hidden={hiddenL} refArea={null} showPosSeries={true} />}
            </div>
          </div>
